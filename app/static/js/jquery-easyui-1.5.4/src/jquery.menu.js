...
(function($){
	...
	function createItem(target, div, options){
		...
		if (!item.hasClass('menu-sep')){
			...
			item.empty().append($('<div class="menu-text-wxg"></div>').html(itemOpts.text));
			if (itemOpts.iconCls){
	...
	function setMenuSize(target, menu){
		...
		if (isNaN(parseInt(width))){
			width = 0;
			menu.find('div.menu-text-wxg').each(function(){
	...
	$.fn.menu.methods = {
		...
		setText: function(jq, param){
			return jq.each(function(){
				...
				$(param.target).children('div.menu-text-wxg').html(param.text);
			});
		},
		/**
